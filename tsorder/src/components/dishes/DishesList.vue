...
export default {
  ...
  created() {
    this.$axios
      .get(
        "https://www.easy-mock.com/mock/5d4bc17712535d268b60eaaf/example/dish"
      )
      .then(res => {
        const keys = Object.keys(res.data.data);
        for (let v of keys) {
          if (res.data.data[v].classify == this.fatherName) {
            this.dishes.push(res.data.data[v]);
          }
        }
      });
  },
  methods:{
	  order(item){
  		...
  		this.$axios({
  			method:'post',
  			url:'https://order-db2d9.firebaseio.com/order.json',
  			data:item
  		})
  		.then(
  			res=>{
...
//   		); 
  ...
  // 		//在点击的时候先去请求一下数据
  // 		this.$axios.get('https://www.easy-mock.com/mock/5d4bc17712535d268b60eaaf/example/dish')
  // 		.then((res)=>{
  // 				//console.log(res);
  ...
  // 					var keys=res.data.data.projects;
